...
def extract_section(job_text: str, section_name: str) -> str:
    ...
    m = re.search(rf"(?ms)^\[{re.escape(section_name)}\]\n(.*?)(?=^\[|\Z)", text)
    return m.group(1) if m else ""


def section_value(section_text: str, key: str, default: str = "") -> str:
    m = re.search(rf"(?m)^{re.escape(key)}=(.*)$", section_text)
...
def parse_job_text(job_text: str) -> dict[str, Any]:
    ...
    row_defs = [
        ...
        ("Special Ex Situ", "Ex Situ Conditioning Optimization", True),
        ("ISRM Algorithm", "ISRM Algorithm", False),
        ("WL Algorithm", "WL Algorithm", False),
        ("RTPC Recipe", "RTPC Recipe", False),
        ("PRC Algorithm", "PRC Algorithm", False),
        ("FT Algorithm", "FT Algorithm", False),
    ]
    ...
    for i in range(number_of_steps):
        ...
        cleaner_rows.append({"index": STATION_LABELS.get(station_id, station_id or f"Step {i}"), "module": station_id or NONE_LABEL, "recipe": recipe})

    return {
        "preMetrology": {"enabled": pre_enabled, "recipe": pre_recipe},
        "polisher": {"route": route_to_polisher, "rows": platen_rows},
        "cleaner": {"route": route_to_cleaner, "numberOfSteps": number_of_steps, "rows": cleaner_rows},
        "postMetrology": {"enabled": post_enabled, "recipe": post_recipe},
    }
