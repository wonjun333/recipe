...
def list_inventory_entries(eqp_id: str, source_path: str | None = None, exts: list[str] | None = None, include_absent: bool = True) -> list[dict[str, Any]]:
    ...
    conn = _connect()
    try:
        sql = 'SELECT * FROM equipment_inventory WHERE eqp_id=?'
...
def get_latest_version(eqp_id: str, source_path: str, name: str) -> dict[str, Any] | None:
    ...
    try:
        row = conn.execute(
            'SELECT * FROM file_versions WHERE eqp_id=? AND source_path=? AND name=? ORDER BY version_id DESC LIMIT 1',
            (eqp_id, _norm_path(source_path), str(name or '').strip()),
        ).fetchone()
        if not row:
            return None
...
def get_inventory_revision(eqp_id: str) -> int:
    ...
    conn = _connect()
    try:
        row = conn.execute('SELECT revision FROM inventory_state WHERE eqp_id=?', (eqp_id,)).fetchone()
