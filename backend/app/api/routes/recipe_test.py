...
from app.services.recipe_cache_store import get_latest_version, get_latest_version_bytes, list_inventory_entries
...
PROCESS_EQP_SNAPSHOT_HASH: dict[str, str] = {}
...
def _inventory_snapshot_payload(eqp_id: str) -> list[list[Any]]:
    payload: list[list[Any]] = []
    for source_kind, cfg in RECIPE_SOURCE_CONFIG.items():
        path = str(cfg.get('path') or '')
        exts = list(cfg.get('exts') or [])
        for row in list_inventory_entries(eqp_id, path, exts=exts, include_absent=True):
            ...
            payload.append([
                ...
                str(row.get('size') or ''),
                1 if bool(row.get('livePresent')) else 0
            ])
    payload.sort(key=lambda x: (str(x[0]), str(x[2]).lower(), str(x[3]), str(x[4])))
    return payload


def get_inventory_snapshot_hash_for_eqp(eqp_id: str) -> str:
    payload = _inventory_snapshot_payload(str(eqp_id or '').strip())
    if not payload:
        return ''
    return hashlib.sha1(json.dumps(payload, ensure_ascii=False, separators=(',', ':')).encode('utf-8')).hexdigest()


def invalidate_eqp_runtime_cache(eqp_id: str) -> None:
    key = str(eqp_id or '').strip()
    if not key:
        return
    BOOTSTRAP_CACHE.pop(key, None)
    for cache_obj in (CAS_CACHE, JOB_CACHE, RECIPE_CACHE, RECIPE_SOURCE_CACHE):
        doomed = [k for k in list(cache_obj.keys()) if isinstance(k, tuple) and len(k) > 0 and str(k[0]) == key]
        for item in doomed:
            cache_obj.pop(item, None)


def ensure_eqp_cache_fresh(eqp_id: str) -> None:
    key = str(eqp_id or '').strip()
    if not key:
        return
    current_hash = get_inventory_snapshot_hash_for_eqp(key)
    prev_hash = PROCESS_EQP_SNAPSHOT_HASH.get(key)
    if prev_hash is None:
        PROCESS_EQP_SNAPSHOT_HASH[key] = current_hash
        return
    if prev_hash != current_hash:
        invalidate_eqp_runtime_cache(key)
    PROCESS_EQP_SNAPSHOT_HASH[key] = current_hash


def _build_cached_recipe_union(eqp_id: str, ftp_ip: str | None = None, ftp_id: str | None = None, ftp_pw: str | None = None) -> list[dict[str, Any]]:
    ...
    for source_kind, cfg in RECIPE_SOURCE_CONFIG.items():
        ...
        path = str(cfg.get('path') or '')
        exts = list(cfg.get('exts') or [])
        items = list_cached_or_live_entries_for_source(eqp_id, path, exts, ftp_ip, ftp_id, ftp_pw)
        for item in items:
            name = str(item.get('name') or '').strip()
            if not name:
                continue
            union[(source_kind, name)] = {
                'id': make_source_recipe_id(source_kind, name),
                'name': name,
                'modifiedAt': str(item.get('modifiedAt') or ''),
                'sourceKind': source_kind,
                'ext': str(item.get('ext') or ''),
                'livePresent': bool(item.get('livePresent'))
            }
    return sorted(union.values(), key=lambda x: (str(x.get('sourceKind') or ''), str(x.get('name') or '').lower()))
...
def is_placeholder_recipe_preview(preview: Any) -> bool:
    ...
    for row in rows:
        ...
        value = str(row.get('Value') or row.get('STATUS') or '').strip().upper()
        if key == 'STATUS' and value == 'PLACEHOLDER_PREVIEW':
            return True
    return False


def normalize_cached_recipe_preview(preview: dict[str, Any], recipe_id: str, recipe_name: str, modified_at: str, source_kind: str) -> dict[str, Any]:
    result = json.loads(json.dumps(preview))
    recipe = dict(result.get('recipe') or {})
    recipe['id'] = recipe_id
    recipe['name'] = recipe_name
    recipe['sourceKind'] = source_kind
    recipe['modifiedAt'] = str(recipe.get('modifiedAt') or modified_at or '').strip()
    result['recipe'] = recipe
    return result


def load_cached_recipe_preview(eqp_id: str, source_path: str, recipe_name: str, recipe_id: str, modified_at: str, source_kind: str) -> dict[str, Any] | None:
    cached = get_latest_version(eqp_id, source_path, recipe_name)
    if not cached:
        return None

    cached_modified_at = str(cached.get('modifiedAt') or '').strip()
    effective_modified_at = str(modified_at or cached_modified_at or '').strip()
    cached_preview = cached.get('preview')

    if isinstance(cached_preview, dict) and not is_placeholder_recipe_preview(cached_preview):
        return normalize_cached_recipe_preview(cached_preview, recipe_id, recipe_name, effective_modified_at, source_kind)

    raw_bytes = get_latest_version_bytes(eqp_id, source_path, recipe_name)
    if raw_bytes:
        preview_context: dict[str, Any] = {'eqpId': eqp_id}
        if recipe_name.lower().endswith('.pol') or recipe_name.lower().endswith('.con'):
            try:
                ...
                preview_context['slurryConfig'] = {}
        reparsed = build_recipe_preview_from_bytes(recipe_id, recipe_name, effective_modified_at, source_kind, raw_bytes, preview_context)
        if reparsed and not is_placeholder_recipe_preview(reparsed):
            return normalize_cached_recipe_preview(reparsed, recipe_id, recipe_name, effective_modified_at, source_kind)
...
def build_source_recipe_content(eqp_id: str, recipe_id: str, source_kind: str, recipe_name: str) -> dict[str, Any]:
    ...
    try:
        ...
        cached_preview = load_cached_recipe_preview(eqp_id, path, recipe_name, recipe_id, modified_at, source_kind)
        if cached_preview:
            return cached_preview

    cached_preview = load_cached_recipe_preview(eqp_id, path, recipe_name, recipe_id, modified_at, source_kind)
    if cached_preview and not is_placeholder_recipe_preview(cached_preview):
        return cached_preview
...
def get_or_bootstrap_eqp(eqp_id: str) -> dict[str, Any]:
    ...
    data = {
        ...
        "recipeList": _build_cached_recipe_union(eqp_id, ftp_ip, ftp_id, ftp_pw),
        "casToJobs": {},
    }
...
def load_recipe_test(req: LoadRequest):
    try:
        ensure_eqp_cache_fresh(req.eqpId)
        ...
        recipe_list = _build_cached_recipe_union(req.eqpId, ftp_ip, ftp_id, ftp_pw)
