<template>
  ...
  >
        ...
        >
          <template v-if="listMode === 'detail'">
            <div
              ...
              @click="emit('toggle-sort', 'jobName')"
            >
              <span class="head-label">File Name</span>
              <span class="sort-ind">{{ sortIndicator('jobName') }}</span>
    ...
    <div class="w97-scroll job-scroll" :ref="setScrollRef" @scroll="emit('body-scroll')">
      <div class="w97-col list-col-wide" v-for="(col, idx) in columns" :key="idx" :style="{ width: columnBlockWidth + 'px' }">
        <ul class="w97-ul">
          <li
            ...
            :class="{ active: selectedIds.includes(item.id), 'detail-row': listMode === 'detail' }"
            @click="emit('item-click', { id: item.id, event: $event })"
            @contextmenu.prevent.stop="emit('item-contextmenu', { id: item.id, event: $event })"
            :ref="(el) => setItemRef(item.id, el)"
          >
            <template v-if="listMode === 'detail'">
              <span class="cell-name name-col" :style="{ width: colWidths.name + 'px' }" :title="item.jobName">
                {{ item.displayName }}
              </span>
            ...
            <template v-else>
              <span class="name-only" :style="{ width: colWidths.name + 'px' }" :title="item.jobName">
                {{ item.displayName }}
              </span>
            </template>
          </li>
...
type SortKey = 'jobName' | 'modifiedAt'
type ResizeColKey = 'name' | 'modifiedAt'
type SortDir = 'asc' | 'desc' | null

type JobListViewItem = {
  id: string
  jobName: string
  displayName: string
...
const props = defineProps({
  ...
  columns: {
    type: Array as PropType<JobListViewItem[][]>,
    default: () => [],
  },
  selectedIds: {
...
const emit = defineEmits<{
  ...
  (e: 'start-resize', payload: { colKey: ResizeColKey; event: MouseEvent }): void
  (e: 'body-scroll'): void
...
.name-only,
.cell-name,
.cell-time{ display:block; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; box-sizing:border-box; min-width:0; height:22px; line-height:18px; padding:2px 6px; }
.cell-name{ padding-right:8px; border-right:1px solid #c6c6c6; }
.cell-time{ padding-left:8px; font-size:12px; font-weight:700; color:#555; }
.attention-job-list{ animation:jobListShake .45s ease 1; }
@keyframes jobListShake{
